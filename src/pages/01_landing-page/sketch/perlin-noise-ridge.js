...
export default function (sketch, ridge) {
    ...
    for (let x = 0; x <= size.width + 10; x += 10) {
        const noise = sketch.map(sketch.noise(perlinXoff, perlinYoff), 0, 1, -40, 40);
        ...
        const y = ridgeBase  - ridgeYoff - noise - oscillation;
        // Set the vertex
        sketch.vertex(x, y);
    ...
    if (ridge.flag) {
        ...
        for (let x = size.width; x >= -15; x -= 10) {
            ...
            const n = sketch.map(sketch.noise(xBaseOff, perlinYoff), 0, 1, -10, 10);
            const newy = ridgeBase + n + 50;
            // Set the vertex
